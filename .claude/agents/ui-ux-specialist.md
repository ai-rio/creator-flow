...
export function ViralContentWidget({ videoId, metrics }: ViralContentProps) {
  return (
    <Card className="p-4 border-l-4 border-l-creator-pink">
      <div className="flex items-center space-x-3">
        <div className="p-2 bg-creator-pink/10 rounded-lg">
          <TrendingUp className="h-5 w-5 text-creator-pink" />
        </div>
      ...
      <div className="flex items-center justify-around py-2">
        {navItems.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            className="flex flex-col items-center space-y-1 p-2 text-gray-600 hover:text-primary-600"
          >
  ...
  return (
    <Card className="max-w-md mx-auto p-6">
      <div className="text-center mb-6">
        <div className="w-16 h-16 bg-creator-pink/10 rounded-full flex items-center justify-center mx-auto mb-4">
          <Zap className="h-8 w-8 text-creator-pink" />
        </div>
        <h2 className="text-xl font-bold text-gray-900">Connect Your TikTok Shop</h2>
        <p className="text-gray-600 mt-2">
          Automate your order fulfillment in under 2 minutes
        </p>
      </div>
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <Button 
            className="w-full bg-creator-pink hover:bg-creator-pink/90"
...
export function PricingCard({ tier, isPopular }: PricingCardProps) {
  return (
    <Card className={cn(
      ...
      isPopular 
        ? "border-creator-pink shadow-lg scale-105" 
        : "border-gray-200"
    )}>
      {isPopular && (
        <Badge className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-creator-pink">
          Most Popular
        </Badge>
      )}
      ...
      <div className="text-center">
        ...
        <p className="text-sm text-gray-600 mt-2">
          Up to {tier.orderLimit} orders/month
        </p>
      </div>
      ...
      <Button 
        className={cn(
          "w-full mt-6",
          isPopular 
            ? "bg-creator-pink hover:bg-creator-pink/90" 
