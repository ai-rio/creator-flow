...
import { Zap, Target, Flame, Bot, Sun, Moon, Palette, DollarSign, Package, Truck, Video, AlertTriangle, TrendingUp } from 'lucide-react';
...
const AnimatedNumber = ({ value, isCurrency = false }) => { const [displayValue, setDisplayValue] = useState(0); useEffect(() => { const controls = animate(0, value, { duration: 1.5, ease: "easeOut", onUpdate(latest) { setDisplayValue(Math.round(latest)); } }); return () => controls.stop(); }, [value]); return (<span>{isCurrency && '$'}{displayValue.toLocaleString()}</span>); };
