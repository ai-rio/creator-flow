...
import { Sun, Moon, Database, Sliders, Zap, Link } from 'lucide-react';
...
const FP020OrderManagementDefinitive = ({ theme }) => {
        ...
        <div className="flex min-h-screen w-full flex-col items-center justify-center p-4 lg:p-8">
            <div className="text-center">
                 ...
                 <p className={`mx-auto mt-4 max-w-3xl text-lg ${theme.textSecondary}`}>
                     CreatorFlow unifies every order into a singular, intelligent pipeline. Hover over a capability to see its direct impact.
                 </p>
            </div>
...
const LiveOrderStream = ({ activePillar, theme }) => {
    ...
    useEffect(() => {
        ...
        const animateScroll = () => {
            scrollAmount += 0.5;
            if (scrollAmount >= stream.scrollHeight - stream.clientHeight) {
                scrollAmount = 0;
            }
    ...
    return (
        <div className={`order-stream relative col-span-full row-span-full h-full w-full overflow-y-scroll rounded-2xl border p-6 backdrop-blur-md md:col-span-4 md:col-start-3 md:row-span-4 ${theme.glassBg} ${theme.border}`}>
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                             {activePillar === 'validation' && order.status === 'FLAGGED' && (
                                <motion.span initial={{opacity:0}} animate={{opacity:1}} className="h-2 w-2 rounded-full bg-red-500" />
                            )}
                            <motion.span 
...
const FeaturePillar = ({ pillar, setActivePillar, theme }) => (
    <motion.div
        style={{ gridArea: pillar.gridArea }}
        className={`hidden rounded-2xl border p-6 backdrop-blur-md md:flex flex-col justify-between ${theme.glassBg} ${theme.border}`}
...
const BlueprintSparks = ({ activePillar, theme }) => {
