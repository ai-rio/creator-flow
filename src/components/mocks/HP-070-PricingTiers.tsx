...
export default function PricingShowcase(): React.JSX.Element {
  ...
  return (
    <section className='flex min-h-screen w-full flex-col items-center justify-center p-4'>
      <div className='mb-12 text-center'>
        <h2 className='bg-gradient-to-br from-slate-800 to-slate-500 bg-clip-text text-4xl font-extrabold tracking-tight text-transparent dark:from-white dark:to-slate-400 md:text-5xl'>
          Find Your Orbit.
        </h2>
...
const BillingToggle = ({ billingCycle, setBillingCycle }: any) => (
  <div className='flex items-center gap-2 rounded-full border border-slate-900/10 bg-slate-200/50 p-1 backdrop-blur-lg dark:border-slate-100/10 dark:bg-black/30'>
    <button
      onClick={() => setBillingCycle('monthly')}
      className='relative rounded-full px-4 py-2 text-sm font-semibold text-slate-800 dark:text-white'
    >
      {billingCycle === 'monthly' && (
        <motion.div
          layoutId='billing-pill'
          className='absolute inset-0 rounded-full bg-white/80 dark:bg-slate-700/50'
        />
      )}
    ...
    <button
      ...
      className='relative flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-slate-800 dark:text-white'
    >
      {billingCycle === 'yearly' && (
        <motion.div
          layoutId='billing-pill'
...
const PricingCard: React.FC<any> = ({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className='absolute inset-0 rounded-2xl border border-slate-900/10 bg-slate-200/50 backdrop-blur-2xl dark:border-slate-100/10 dark:bg-black/30'
        animate={{
      ...
      <div className='absolute inset-0 flex flex-col justify-between rounded-2xl p-6'>
        <div>
          <h3 className='text-2xl font-bold text-slate-800 dark:text-white'>{name}</h3>
          <p className='mt-1 text-sm text-slate-600 dark:text-slate-400'>{description}</p>
          <div className='mt-4 flex items-baseline text-4xl font-extrabold text-slate-800 dark:text-white'>
        ...
        <motion.ul className='space-y-2'>
          {features.map((feature: any, i: any) => (
            ...
            >
              <Check className='h-4 w-4' style={{ color: accentColor.light }} /> {feature}
            </motion.li>
          ))}
        </motion.ul>
        <motion.button
          className='mt-4 w-full rounded-lg py-2 font-bold text-white'
