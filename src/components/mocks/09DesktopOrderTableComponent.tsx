...
const TableHeader = ({ children }: any) => <th className="p-4 text-sm font-semibold text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-800">{children}</th>;
const TableCell = ({ children, className = '' }: any) => <td className={`p-4 text-sm text-gray-800 dark:text-gray-200 border-b border-gray-200/50 dark:border-gray-800/50 ${className}`}>{children}</td>;
                ...
                <motion.tbody variants={containerVariants} initial="hidden" animate="visible">
                  {mockOrderData.map(order => {
                    ...
                    return (
                      ...
                      >
                        <TableCell>
                          <div className={`w-8 h-8 flex items-center justify-center rounded-full ${(priorityIcons as any)[order.priority].className}`}>
