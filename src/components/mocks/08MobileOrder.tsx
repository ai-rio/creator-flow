...
const SearchIcon = ({ className = '' }: any) => <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${className}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>;
const BackIcon = ({ className = '' }: any) => <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${className}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>;
...
// --- Helper Components ---
const GlassPane = React.forwardRef<any, any>(({ children, className }: any, ref: any) => ( <div ref={ref} className={`relative rounded-2xl border bg-white/80 dark:bg-black/20 border-slate-300 dark:border-white/10 backdrop-blur-xl transition-all duration-300 ${className}`}>{children}</div>));
