...
const IntelligenceBriefingCard: React.FC<any> = ({ insights }) => (
  <motion.div
    initial={{ opacity: 0, y: 50 }}
    animate={{ opacity: 1, y: 0 }}
  ...
  >
    ...
    <GlassPane className='rounded-xl p-4'>
      ...
      <div className='mb-4 flex items-center gap-2'>
        {' '}
        <BrainCircuit className='text-slate-600 dark:text-slate-400' />{' '}
        ...
        {insights.map((i: any) => (
          ...
          >
            ...
            <div>
              ...
              <p className='text-sm text-slate-600 dark:text-slate-400'>{i.subtitle}</p>{' '}
            </div>{' '}
          </motion.div>
        ))}{' '}
      </motion.div>{' '}
          ...
          {navItems.map((item) => (
            <div key={item.id} className='relative flex flex-1 justify-center'>
              {item.isFab ? (
                <motion.button
                  whileTap={{ scale: 0.9 }}
                  className='-translate-y-6 rounded-full bg-purple-700 p-4 text-white shadow-lg shadow-purple-500/50'
                >
                  <Plus size={32} />
                </motion.button>
              ) : (
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className={`relative z-10 rounded-full p-3 transition-colors ${
                    activeTab === item.id ? 'text-teal-800 dark:text-teal-300' : 'text-slate-500'
                  }`}
                >
                  <item.icon size={24} />
                  {activeTab === item.id && (
                    <motion.div
                      ...
                      className='absolute inset-0 -z-10 rounded-full bg-teal-200/50 dark:bg-teal-900/50'
