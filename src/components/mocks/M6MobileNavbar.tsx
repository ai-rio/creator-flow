...
import { AlertTriangle, BarChart3, Bot, BrainCircuit, Clock, Crosshair, DollarSign, Download, Flame, LayoutDashboard, Moon, Package, Palette, Plus,Sparkles, Sun, Target, TrendingUp, Truck, Video, Wand2, Zap } from 'lucide-react';
...
const intelligenceBriefing = [ { id: 1, type: 'performance', title: 'Content ROI: $247/video avg', subtitle: 'Focus on short-form unboxing videos.' }, { id: 2, type: 'trend', title: 'Growth: 340%/yr sustainable', subtitle: 'Based on current supply chain capacity.' }, { id: 3, type: 'ai', title: 'Next: EU expansion ready', subtitle: 'AI recommends targeting Germany & France.' }, ];
...
const AnimatedNumber = ({ value, isCurrency = false }) => { const [val, setval] = useState<any>(0); useEffect(() => { const anim = animate(0, value, { duration: 1.5, ease: "easeOut", onUpdate(l) { setVal(Math.round(l)); } }); return () => anim.stop(); }, [value]); return (<span>{isCurrency && '$'}{val.toLocaleString()}</span>); };
...
const CircularProgress = ({ percentage }) => { const r = 40, c = 2 * Math.PI * r; const s = useSpring(0, { stiffness: 50, damping: 20 }); const t = useTransform(s, (l) => `${Math.round(l)}%`); useEffect(() => { s.set(percentage); }, [percentage, s]); return (<div className="relative w-24 h-24"><svg className="w-full h-full" viewBox="0 0 100 100"><circle cx="50" cy="50" r={r} strokeWidth="10" className="stroke-slate-300/50 dark:stroke-slate-700/50" fill="transparent" /><motion.circle cx="50" cy="50" r={r} strokeWidth="10" className="stroke-teal-800 dark:stroke-teal-400" fill="transparent" strokeDasharray={c} strokeLinecap="round" transform="rotate(-90 50 50)" style={{ strokeDashoffset: useTransform(s, p => c - (p / 100) * c) }} /></svg><motion.div className="absolute inset-0 flex items-center justify-center text-2xl font-bold text-slate-900 dark:text-slate-100">{t}</motion.div></div>); };
...
const InsightIcon = ({ type }) => { const i = { performance: <Crosshair className="text-teal-800 dark:text-teal-400" size={20} />, trend: <TrendingUp className="text-amber-600 dark:text-amber-400" size={20} />, ai: <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.7, 1, 0.7] }} transition={{ duration: 2, repeat: Infinity }}><Sparkles className="text-purple-700 dark:text-purple-400" size={20} /></motion.div> }; return i[type] || null; };
...
const StrategicCommandCard: React.FC<any> = ({ alerts  }) => ( <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 100, damping: 20, delay: 0.8 }}> <GlassPane className="p-4 rounded-xl"> <div className="flex items-center gap-2 mb-4"> <Target className="text-slate-600 dark:text-slate-400" /> <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Strategic Command</h2> </div> <motion.div className="space-y-3 mb-4" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }}> {alerts.map(a => <motion.div key={a.id} variants={{ hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1 } }} className="flex items-center justify-between p-2 bg-slate-200/50 dark:bg-slate-900/50 rounded-lg"><div className="flex items-center gap-3"><AlertIcon type={a.type} /><p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{a.text}</p></div><span className="text-xs font-medium text-slate-500 bg-slate-300/50 dark:bg-slate-700/50 px-2 py-0.5 rounded-full">{a.source}</span></motion.div>)} </motion.div> <div className="grid grid-cols-3 gap-2"> <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="col-span-2 bg-teal-700 dark:bg-teal-500 text-white dark:text-slate-900 font-bold py-2 px-3 text-sm rounded-lg shadow-md">Auto-Scale</motion.button> <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="bg-slate-200/80 dark:bg-slate-800/80 text-slate-800 dark:text-slate-200 font-bold py-2 px-3 text-sm rounded-lg">Manual</motion.button> </div> </GlassPane> </motion.div> );
const LiberationOrchestraCard: React.FC<any> = ({ stats  }) => ( <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 100, damping: 20, delay: 1.1 }}> <GlassPane className="p-4 rounded-xl"> <div className="flex items-center gap-2 mb-4"> <Bot className="text-slate-600 dark:text-slate-400" /> <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Liberation Orchestra</h2> </div> <div className="flex items-center justify-between gap-4"> <div className="flex-1 space-y-3"> <div className="flex items-center gap-3"><Clock className="text-purple-700 dark:text-purple-400" size={24} /><div><div className="text-2xl font-bold text-slate-900 dark:text-slate-100"><AnimatedNumber value={stats.hoursSaved} /></div><div className="text-sm text-slate-600 dark:text-slate-400">hours saved</div></div></div> <div className="flex items-center gap-3"><Wand2 className="text-purple-700 dark:text-purple-400" size={24} /><div><div className="text-2xl font-bold text-slate-900 dark:text-slate-100"><AnimatedNumber value={stats.tasksAutomated} /></div><div className="text-sm text-slate-600 dark:text-slate-400">tasks automated</div></div></div> </div> <div className="flex-shrink-0"><CircularProgress percentage={stats.flowHealth} /><p className="text-xs text-center mt-1 text-slate-600 dark:text-slate-400">Flow Health</p></div> </div> <div className="grid grid-cols-2 gap-2 mt-4"> <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="bg-teal-700 dark:bg-teal-500 text-white dark:text-slate-900 font-bold py-2 px-3 text-sm rounded-lg shadow-md">View Orchestra</motion.button> <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="bg-slate-200/80 dark:bg-slate-800/80 text-slate-800 dark:text-slate-200 font-bold py-2 px-3 text-sm rounded-lg">Optimize</motion.button> </div> </GlassPane> </motion.div> );
const IntelligenceBriefingCard: React.FC<any> = ({ insights  }) => ( <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 100, damping: 20, delay: 1.4 }}> <GlassPane className="p-4 rounded-xl"> <div className="flex items-center gap-2 mb-4"> <BrainCircuit className="text-slate-600 dark:text-slate-400" /> <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Your Intelligence Briefing</h2> </div> <motion.div className="space-y-4" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.15 } } }}> {insights.map(i => ( <motion.div key={i.id} variants={{ hidden: { x: -20, opacity: 0 }, visible: { x: 0, opacity: 1 } }} className="flex items-start gap-3"> <div className="mt-1"><InsightIcon type={i.type} /></div> <div> <p className="font-bold text-slate-900 dark:text-slate-100">{i.title}</p> <p className="text-sm text-slate-600 dark:text-slate-400">{i.subtitle}</p> </div> </motion.div> ))} </motion.div> <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-300/50 dark:border-slate-700/50"> <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="bg-teal-700 dark:bg-teal-500 text-white dark:text-slate-900 font-bold py-2 px-3 text-sm rounded-lg shadow-md">Strategic Dashboard</motion.button> <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="flex items-center justify-center gap-2 bg-slate-200/80 dark:bg-slate-800/80 text-slate-800 dark:text-slate-200 font-bold py-2 px-3 text-sm rounded-lg"><Download size={14} />Export</motion.button> </div> </GlassPane> </motion.div> );

// --- M6: The Mobile Navigation Bar ---
const navItems = [
                    ...
                    {navItems.map(item => (
                        <div key={item.id} className="relative flex-1 flex justify-center">
                             {item.isFab ? (
                                <motion.button
                                    whileTap={{ scale: 0.9 }}
                                    className="-translate-y-6 bg-purple-700 text-white rounded-full p-4 shadow-lg shadow-purple-500/50"
                                >
                                    <Plus size={32} />
                                </motion.button>
                            ) : (
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.9 }}
                                    className={`relative z-10 p-3 rounded-full transition-colors ${activeTab === item.id ? 'text-teal-800 dark:text-teal-300' : 'text-slate-500'}`}
                                >
                                    <item.icon size={24} />
                                    {activeTab === item.id && (
                                        <motion.div
                                            layoutId="active-nav-indicator"
                                            className="absolute inset-0 bg-teal-200/50 dark:bg-teal-900/50 rounded-full -z-10"
