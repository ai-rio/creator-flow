...
import { ArrowRight, Bolt, Moon, Sun } from 'lucide-react';
...
export default function App(): React.JSX.Element {
  ...
  return (
    <div className={`font-sans ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <main className='relative z-0 px-4 pb-12 pt-48'>
        <div className='mx-auto max-w-4xl space-y-8'>
          <h1 className={`text-center text-4xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>
            Content Example
          </h1>
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className={`h-48 w-full rounded-2xl ${theme === 'dark' ? 'bg-slate-800/50' : 'bg-white/50'} border ${
                theme === 'dark' ? 'border-slate-100/10' : 'border-slate-900/10'
              }`}
            />
          ))}
        </div>
...
const HP010HeaderDefinitive: React.FC<any> = ({ theme }: any) => {
...
const Navigation: React.FC<any> = ({ isScrolled, theme }: any) => {
            ...
            {navLinks.map((link) => (
              <motion.a
                ...
                className={`text-sm font-medium ${
                  theme === 'dark' ? 'text-slate-300 hover:text-white' : 'text-slate-700 hover:text-black'
                }`}
                whileTap={{ scale: 0.95 }}
              >
                {link}
    ...
    <AnimatePresence mode='popLayout'>
      {isScrolled ? (
        ...
        <motion.button
          ...
          animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0.5 }}
          className='supernova-cta relative rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white'
...
const ThemeToggleButton = ({ theme, setTheme }: any) => (
  <motion.button
    ...
    className={`fixed bottom-4 right-4 z-50 flex h-12 w-12 items-center justify-center rounded-full border ${
      theme === 'dark'
