...
function AppContent() {
    ...
    const KpiCard: React.FC<any> = ({ title, value, icon, refProp  }) => (<GlassPane ref={refProp} className="p-4"><div className="flex justify-between items-start"><div className="flex flex-col"><p className="text-sm text-slate-600 dark:text-slate-400">{title}</p><p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{value}</p></div><div className="text-slate-500 dark:text-slate-400">{icon}</div></div></GlassPane>);
    const CrisisCard: React.FC<any> = ({ refProp  }) => (<GlassPane ref={refProp} className="p-6 h-full"><h2 className="text-xl font-bold text-amber-600 dark:text-amber-300 mb-4 flex items-center gap-2"><AlertIcon/> Crisis Command</h2><p className="text-slate-600 dark:text-slate-400">This is where you manage viral events...</p></GlassPane>);
    const MasterpieceCard: React.FC<any> = ({ refProp  }) => (<GlassPane ref={refProp} className="p-6 h-full"><h2 className="text-xl font-bold text-slate-900 dark:text-white">Revenue Masterpiece</h2><p className="text-slate-600 dark:text-slate-400">Your interactive financial landscape...</p></GlassPane>);
