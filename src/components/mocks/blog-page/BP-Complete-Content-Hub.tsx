...
const Search = ({ size = 24, className = '' }: any) => (
  <svg
...
// --- Component 1: Featured Dossier Hero ---
const FeaturedDossierHero: React.FC<any> = ({ post, theme = 'dark' }: any) => {
  const themeClasses = {
    dark: {
      glass: 'bg-black/50 border-slate-800/80',
      featuredLabel: 'text-teal-400',
      title: 'text-white',
      metaText: 'text-slate-300',
      cta: 'bg-teal-500 hover:bg-teal-400 text-white',
      focusRing: 'focus:ring-teal-400',
    },
    light: {
      glass: 'bg-white/60 border-slate-300',
      featuredLabel: 'text-purple-600 font-bold',
  ...
  if (!post) return null; // Don't render if there's no post

  return (
    <div className='group relative mb-12 h-[60vh] max-h-[600px] min-h-[450px] w-full overflow-hidden rounded-2xl'>
      <div
      ...
      <div className='relative flex h-full flex-col justify-end p-8 md:p-12'>
        <div
          className={`w-full rounded-xl border p-6 backdrop-blur-xl transition-colors duration-300 md:w-2/3 md:p-8 lg:w-1/2 ${currentTheme.glass}`}
        >
          <p className={`text-sm font-bold uppercase tracking-wider ${currentTheme.featuredLabel}`}>Featured Dossier</p>
          <h2 className={`mt-2 text-3xl font-black leading-tight md:text-4xl ${currentTheme.title}`}>{post.title}</h2>
          <div className={`mt-4 flex items-center text-sm ${currentTheme.metaText}`}>
          ...
          <a
            href={post.href}
            className={`mt-6 inline-flex items-center rounded-lg px-6 py-3 font-semibold transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
              currentTheme.cta
            } ${currentTheme.focusRing} ${theme === 'dark' ? 'focus:ring-offset-black' : 'focus:ring-offset-white'}`}
          >
            Read the Briefing
...
export default function App(): React.JSX.Element {
  ...
  return (
    <div
      className={`min-h-screen p-4 font-sans transition-colors duration-500 sm:p-8 md:p-12 ${
    ...
    >
      <div className='mx-auto max-w-7xl'>
        ...
        <FeaturedDossierHero post={featuredPost} theme={theme} />
        ...
        <div className='mb-8 text-left'>
          <h1 className={`text-3xl font-black md:text-4xl ${theme === 'dark' ? 'text-slate-100' : 'text-slate-900'}`}>
