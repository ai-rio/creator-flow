...
const ResultTooltip = ({ theme, onReset }: any) => (
  <motion.div
    ...
    className={`absolute left-1/2 top-1/2 w-80 -translate-x-1/2 -translate-y-1/2 rounded-2xl border p-4 backdrop-blur-md ${theme.glassBg} ${theme.border}`}
  >
    <motion.button
      onClick={onReset}
      className={`absolute -right-3 -top-3 flex h-7 w-7 items-center justify-center rounded-full border ${theme.border} ${theme.glassBg}`}
    ...
    <div className='text-center'>
      <h4 className={`text-lg font-bold ${theme.textPrimary}`}>Optimal Route Found</h4>
