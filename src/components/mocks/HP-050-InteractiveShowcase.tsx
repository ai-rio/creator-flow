...
// --- Main Component ---
export default function InteractiveShowcaseLightspeed(): React.JSX.Element {
      ...
      <div ref={targetRef} className='relative h-[300vh]'>
        ...
        >
          ...
          <motion.div className='absolute z-10 text-center' style={{ opacity: headlineOpacity, y: headlineY }}>
            <h2 className='katana-heading-lightspeed text-6xl md:text-8xl'>The Data Weave.</h2>
          </motion.div>
          ...
          >
            <h3 className={`text-xl font-bold md:text-2xl ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}>
...
const KPI_Card: React.FC<any> = ({ title, value, Icon, i, scrollYProgress, theme }: any) => {
  ...
  const cardClasses =
    theme === 'dark' ? 'bg-black/20 border-slate-100/10 text-white' : 'bg-white/40 border-slate-900/10 text-slate-800';
  return (
    <motion.div
      style={{ translateZ: z, opacity }}
      className={`row-span-1 flex flex-col justify-between rounded-2xl border p-6 backdrop-blur-md ${cardClasses}`}
    >
      <div>
        <Icon className={`h-6 w-6 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`} />
...
const Revenue_Card: React.FC<any> = ({ scrollYProgress, theme }: any) => {
  ...
  return (
    <motion.div
      style={{ translateZ: z, opacity }}
      className={`col-span-2 row-span-2 rounded-2xl border p-6 backdrop-blur-md ${cardClasses}`}
    >
...
const Automation_Card: React.FC<any> = ({ scrollYProgress, theme }: any) => {
  ...
  return (
    <motion.div
      style={{ translateZ: z, opacity }}
      className={`col-span-1 row-span-1 rounded-2xl border p-6 backdrop-blur-md ${cardClasses}`}
    >
      <Zap className='h-8 w-8 text-purple-400' />
      <p className='mt-4 text-xl font-bold'>Automation Liberation</p>
      <p className={`${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'} mt-2`}>
        Saving an average of{' '}
        <span className={`${theme === 'dark' ? 'text-white' : 'text-slate-900'} font-bold`}>12 hours</span> per week.
      </p>
    </motion.div>
  );
};
...
const ParticleCanvas: React.FC<any> = ({ theme }: any) => {
