...
// --- Lucide Icons ---
const ChevronUp = ({ size = 16, className = '' }: any) => (
  <svg
...
const ChevronDown = ({ size = 16, className = '' }: any) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    width={size}
    height={size}
...
const AC_DataTable: React.FC<any> = ({ columns, data, theme = 'dark' }: any) => {
  const cardRef = useRef<any>(null);
  const tbodyRef = useRef<any>(null);
  const scanlineRef = useRef<any>(null); // Ref for the scanline
  ...
  const handleMouseMove = (e: any) => {
    ...
    (cardRef.current as any).style.setProperty('--mouse-x', `${e.clientX - rect.left}px`);
    (cardRef.current as any).style.setProperty('--mouse-y', `${e.clientY - rect.top}px`);
  };

  const handleTbodyMouseMove = (e: any) => {
    if (!(tbodyRef.current as any) || !(scanlineRef.current as any)) return;
    const rect = (tbodyRef.current as any).getBoundingClientRect();
    const y = e.clientY - rect.top;
    (scanlineRef.current as any).style.transform = `translateY(${y - 24}px)`;
    (scanlineRef.current as any).style.opacity = '1';
  };

  const handleTbodyMouseLeave = () => {
    if (scanlineRef.current as any) {
      (scanlineRef.current as any).style.opacity = '0';
    }
  };

  useEffect(() => {
    if ((tbodyRef.current as any) && (scanlineRef.current as any)) {
      (scanlineRef.current as any).style.top = `${(tbodyRef.current as any).offsetTop}px`;
    }
  }, [data]); // Recalculate if data changes

  const sortedData = useMemo(() => {
    if (!(sortConfig as any).key) return data;
    const sorted = [...data].sort((a: any, b: any) => {
      const valA = (a as any)[(sortConfig as any).key];
      const valB = (b as any)[(sortConfig as any).key];
    ...
    return (sortConfig as any).direction === 'ascending' ? sorted : sorted.reverse();
  }, [data, sortConfig]);

  const requestSort = (key: any) => {
    let direction = 'ascending';
    if ((sortConfig as any).key === key && (sortConfig as any).direction === 'ascending') {
                ...
                {columns.map((col: any) => (
                  <th
                    ...
                    className={`datatable-th datatable-header-cell p-4 text-xs font-semibold uppercase tracking-wider ${
                      (sortConfig as any).key === col.accessor ? 'is-active ' + currentTheme.activeHeader : ''
                    }`}
                  >
                    <div className='flex items-center gap-2'>
                      {col.header}
                      <span className='sort-icon'>
                        {(sortConfig as any).key === col.accessor &&
                          ((sortConfig as any).direction === 'ascending' ? <ChevronUp /> : <ChevronDown />)}
                      </span>
                    </div>
                  </th>
                ))}
