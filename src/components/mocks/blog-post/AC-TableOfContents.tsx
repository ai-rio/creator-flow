...
const useIntersectionObserver = (setActiveId: any, options: any) => {
...
const AC_TableOfContents: React.FC<any> = ({ headings, theme = 'dark' }: any) => {
  const cardRef = useRef(null);
  const listRef = useRef(null);
  const [activeId, setActiveId] = useState<string>('');
  const [reticleStyle, setReticleStyle] = useState<any>({});
  ...
  useEffect(() => {
    ...
    const headingElements = headings.map((h: any) => document.getElementById(h.id)).filter(Boolean);
    if (headingElements.length > 0 && (observer as any).current) {
      headingElements.forEach((el: any) => (observer.current as any).observe(el));
    }
    return () => (observer.current as any)?.disconnect();
  }, [headings, observer]);

  useEffect(() => {
    if (!activeId || !(listRef.current as any)) return;

    const activeLink = (listRef.current as any).querySelector(`a[href="#${activeId}"]`);
    if (activeLink) {
      const listItem = activeLink.parentElement;
      setReticleStyle({
        transform: `translateY(${(listItem as any).offsetTop}px)`,
        height: `${(listItem as any).offsetHeight}px`,
      });

      const reticleEl = (listRef.current as any).querySelector('.targeting-reticle');
      if (reticleEl) {
