...
const Info = ({ size = 24, className = '' }: any) => (
  <svg
...
const AlertTriangle = ({ size = 24, className = '' }: any) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    width={size}
    height={size}
...
const XOctagon = ({ size = 24, className = '' }: any) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    width={size}
    height={size}
...
const AC_Callout: React.FC<any> = ({ children, title, variant = 'info', theme = 'dark' }: any) => {
  const variantConfig = {
  ...
  const { Icon, ...variantClasses } = (variantConfig as any)[variant];
  const currentVariantClass = (variantClasses as any)[theme];
    ...
    <div className={`min-h-screen p-8 font-sans transition-colors duration-500 sm:p-12 ${(pageThemes as any)[theme]}`}>
      <div className='mx-auto max-w-3xl'>
        ...
        <AC_Callout variant='info' title='Informational Briefing' theme={theme}>
          This callout uses the theme&apos;s primary accent color. It&apos;s designed to provide helpful context or
          supplementary details that support the main content.
        </AC_Callout>
