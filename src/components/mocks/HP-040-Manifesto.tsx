...
export default function ManifestoShowcase(): React.JSX.Element {
  ...
  return (
    <div className='flex min-h-screen items-center justify-center overflow-hidden bg-slate-100 p-4 font-sans transition-colors duration-300 dark:bg-[#0A090F]'>
...
const ManifestoCard: React.FC<any> = ({ item, onClick }: any) => {
  ...
  return (
    <motion.div
      ...
      className='relative flex aspect-[3/4] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-2xl border border-slate-900/10 bg-slate-200/50 p-8 text-center dark:border-slate-100/10 dark:bg-black/20'
    >
      <motion.div
        className='absolute inset-0 -z-10'
        style={{
          background: `radial-gradient(circle at 50% 50%, ${(colors as any)[item.color].dark}40, transparent 80%)`,
        }}
        animate={{ scale: isHovered ? [1, 1.3, 1] : 1, opacity: isHovered ? 1 : 0.5 }}
        transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div className='z-10' layoutId={`icon-${item.id}`}>
        <item.Icon className='h-12 w-12 text-slate-700 dark:text-slate-300' />
      </motion.div>
      <AnimatePresence>
        {isHovered && (
          <motion.div
          ...
          >
            <motion.h3 layoutId={`title-${item.id}`} className='text-2xl font-bold text-slate-900 dark:text-slate-100'>
              {item.title}
            </motion.h3>
            <motion.p layoutId={`tagline-${item.id}`} className='mt-2 text-slate-600 dark:text-slate-400'>
              {item.tagline}
            </motion.p>
          </motion.div>
        )}
    ...
    <AnimatePresence>
      {selectedItem && (
        ...
        >
          ...
          <motion.div
            layoutId={selectedId}
            className='relative w-full max-w-2xl rounded-2xl border border-slate-900/10 bg-slate-200/80 p-8 dark:border-slate-100/10 dark:bg-black/50'
          >
            <motion.button
            ...
            <div className='flex items-start gap-6'>
              <motion.div layoutId={`icon-${selectedId}`} className='rounded-lg p-2'>
                <selectedItem.Icon className='h-12 w-12 text-slate-700 dark:text-slate-300' />
              </motion.div>
              <div>
                <motion.h3
