...
const BorderBeam: React.FC<any> = ({ className }) => (
  <div className={`pointer-events-none absolute inset-0 overflow-hidden rounded-2xl ${className}`}>
    ...
    />
    <motion.div
      className='absolute right-0 top-0 h-full w-1 bg-gradient-to-b from-transparent via-teal-400 to-transparent'
      initial={{ y: '-100%' }}
      animate={{ y: '100%' }}
      transition={{ duration: 2, repeat: Infinity, ease: 'linear', delay: 1.5 }}
    />
    <motion.div
      className='absolute bottom-0 right-0 h-1 w-full bg-gradient-to-l from-transparent via-teal-400 to-transparent'
      initial={{ x: '100%' }}
      animate={{ x: '-100%' }}
      transition={{ duration: 2, repeat: Infinity, ease: 'linear', delay: 2.5 }}
    />
    <motion.div
      className='absolute left-0 top-0 h-full w-1 bg-gradient-to-t from-transparent via-teal-400 to-transparent'
      initial={{ y: '100%' }}
      animate={{ y: '-100%' }}
...
const KpiCard: React.FC<any> = ({ title, value, change, icon, variants }) => (
  <motion.div variants={variants}>
    <GlassPane className='h-full p-4'>
      ...
      <p className='mt-2 text-sm text-teal-600 dark:text-teal-400'>{change}</p>
    </GlassPane>
  </motion.div>
);

const businessArtData = [
  { name: 'Revenue', value: 12847, max: 15000 },
  ...
  return (
    <motion.div variants={variants}>
      <GlassPane className='flex h-full flex-col p-6'>
          ...
          <ResponsiveContainer width='100%' height='100%'>
            <BarChart data={businessArtData} layout='vertical' margin={{ top: 0, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey='value' barSize={20} radius={[0, 10, 10, 0]}>
                {businessArtData.map((entry, index) => (
        ...
        <div className='mt-4 grid grid-cols-3 gap-2 border-t border-slate-300/70 pt-4 text-center dark:border-slate-700/50'>
          <div>
            <p className='text-xl font-bold text-slate-900 dark:text-white'>$12.8k</p>
            <p className='text-xs text-slate-600 dark:text-slate-400'>Revenue</p>
          </div>
          <div>
            <p className='text-xl font-bold text-slate-900 dark:text-white'>347</p>
          ...
          <div>
            ...
            <p className='text-xs text-slate-600 dark:text-slate-400'>Health</p>
          </div>
        </div>
      </GlassPane>
    </motion.div>
    ...
    <GlassPane className='relative h-full p-6'>
      ...
      <div className='relative z-10'>
        <h2 className='mb-4 flex items-center gap-2 text-xl font-bold text-amber-600 dark:text-amber-300'>
          <AlertIcon /> Strategic Command Center
        </h2>
        <div className='space-y-4'>
          <p className='font-bold text-slate-900 dark:text-white'>VIRAL ALERT: Video #xyz789</p>
          <p className='text-2xl font-bold text-slate-900 dark:text-white'>+ $8,921 in 6h (347 orders)</p>
          <p className='text-md text-slate-600 dark:text-slate-300'>Market Share: +2.3% today</p>
          <div className='flex flex-col gap-4 pt-2 sm:flex-row'>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className='w-full rounded-lg bg-slate-900 px-5 py-2 font-bold text-white dark:bg-white dark:text-slate-900'
            >
              Strategic Response
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className='w-full rounded-lg border border-slate-500/50 bg-transparent px-5 py-2 font-bold transition-colors hover:bg-slate-500/20 dark:border-slate-600 dark:hover:bg-slate-700/50'
            >
              Scale Up
            </motion.button>
          </div>
    ...
    <GlassPane className='flex h-full flex-col justify-between p-6'>
      ...
      <div className='space-y-4'>
        <div className='flex items-center gap-3'>
          <ClockIcon />
          <div>
            <p className='text-2xl font-bold text-slate-900 dark:text-white'>47 hours saved</p>
        ...
        <div className='flex items-center gap-3'>
          <CheckCircleIcon />
          <div>
            <p className='text-2xl font-bold text-slate-900 dark:text-white'>89 orders processed</p>
            <p className='text-sm text-slate-600 dark:text-slate-400'>while you slept</p>
          </div>
        </div>
      </div>
      <div className='mt-4 border-t border-slate-300/70 pt-4 dark:border-slate-700/50'>
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className='w-full rounded-lg border border-slate-500/50 bg-transparent px-5 py-2 font-bold transition-colors hover:bg-slate-500/20 dark:border-slate-600 dark:hover:bg-slate-700/50'
    ...
    <GlassPane className='flex h-full flex-col justify-between p-6'>
      ...
      <div className='space-y-4'>
        <div className='flex items-center gap-3'>
          <TargetIcon />
          <div>
            <p className='text-2xl font-bold text-slate-900 dark:text-white'>$247 (+12%)</p>
            <p className='text-sm text-slate-600 dark:text-slate-400'>Customer LTV</p>
          </div>
        </div>
        <div className='flex items-center gap-3'>
          <GrowthIcon />
          <div>
            <p className='text-2xl font-bold text-slate-900 dark:text-white'>340%/yr</p>
            <p className='text-sm text-slate-600 dark:text-slate-400'>Growth Trajectory</p>
          </div>
        </div>
        <div className='flex items-center gap-3'>
          <StarIcon />
          <div>
            <p className='text-2xl font-bold text-slate-900 dark:text-white'>Next Opportunity</p>
            <p className='text-sm text-teal-600 dark:text-teal-400'>AI Rec: EU Market Expansion</p>
          </div>
        </div>
      </div>
      <div className='mt-4 border-t border-slate-300/70 pt-4 dark:border-slate-700/50'>
...
const Sidebar = ({ isCollapsed, isMobile, setMobileOpen }) => {
  ...
  const content = (
    <GlassPane className='flex h-full flex-col justify-between p-4'>
      <div>
        <div
          className={`mb-12 flex items-center gap-3 ${isCollapsed && !isMobile ? 'justify-center' : 'justify-start'}`}
        >
  ...
  if (isMobile) {
    return (
      <motion.div
        ...
        className='fixed inset-y-0 left-0 z-50 h-screen w-64 p-4'
  ...
  return (
    <div className='flex items-center justify-between'>
      <div className='flex items-center gap-4'>
        ...
        <div>
          <h1 className='text-2xl font-bold text-slate-900 dark:text-white lg:text-3xl'>CEO Dashboard</h1>
          <p className='text-sm text-slate-600 dark:text-slate-400 lg:text-base'>At-a-glance command center.</p>
        </div>
      </div>
      <button
...
const AppContent = () => {
  ...
  return (
    <div className='flex h-screen min-h-screen flex-col gap-4 overflow-hidden bg-slate-100 p-4 font-sans text-slate-900 dark:bg-[#0A090F] dark:text-slate-200 lg:flex-row'>
      <AnimatePresence>
      ...
      <main className='flex flex-1 flex-col overflow-auto'>
        ...
        <motion.div
          ...
          initial='hidden'
          animate='visible'
          className='grid flex-grow grid-cols-1 gap-6 py-6 lg:grid-cols-2 xl:grid-cols-4'
        >
