...
import { AlertTriangle, Bot, BrainCircuit, Clock, Crosshair, DollarSign, Download,Flame, Moon, Package, Palette, Sparkles, Sun, Target, TrendingUp, Truck, Video, Wand2, Zap } from 'lucide-react';
...
const intelligenceBriefing = [
    { id: 1, type: 'performance', title: 'Content ROI: $247/video avg', subtitle: 'Focus on short-form unboxing videos.' },
                ...
                <motion.div className="space-y-4" variants={containerVariants} initial="hidden" animate="visible">
                    {insights.map(insight => (
                        <motion.div key={insight.id} variants={itemVariants} className="flex items-start gap-3">
                            <div className="mt-1"><InsightIcon type={insight.type} /></div>
                            <div>
                 ...
                 <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-300/50 dark:border-slate-700/50">
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="bg-teal-700 dark:bg-teal-500 text-white dark:text-slate-900 font-bold py-2 px-3 text-sm rounded-lg shadow-md">Strategic Dashboard</motion.button>
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }} className="flex items-center justify-center gap-2 bg-slate-200/80 dark:bg-slate-800/80 text-slate-800 dark:text-slate-200 font-bold py-2 px-3 text-sm rounded-lg"><Download size={14} />Export</motion.button>
                </div>
            </GlassPane>
        </motion.div>
    );
